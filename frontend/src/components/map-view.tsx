...
import { emptyGeoJSON, outsideMask, insideBasin } from "@/lib/basin-focus";
import type { FeatureCollection } from "geojson";
import type { GeoJSONSource } from "maplibre-gl";
import Icon from "./icon";
import { configureMapWorker } from "@/lib/map-worker";
import { qualityColors } from "@/lib/demo-data";
...
type Props = {
  ...
  onSelect: (station: StationSnapshot) => void; onOpenChart: () => void;
...
export default function MapView(props: Props) {
  ...
  const geoData = useRef<GeoAssets>({ regencies: emptyGeoJSON, eastRivers: emptyGeoJSON, welangRivers: emptyGeoJSON, subdas: emptyGeoJSON, basin: emptyGeoJSON });
  const [hasBasin,setHasBasin] = useState(false);
  ...
  const popupStation = useRef<{ name: string; menu: boolean } | null>(null);
  ...
  useEffect(() => {
    ...
    fitAll.current = (duration = 0) => {
      ...
      const coordinates = boundsOf(geoData.current[preset.asset]) ?? preset.fallbackBounds;
    ...
    showPopup.current = (name, menu) => {
      ...
      popupStation.current = { name, menu };
      ...
      stationCode.className = "popup-station-code";
      stationCode.textContent = `WL-${station.name.replace(/[^a-z0-9]/gi, "").slice(0, 8).toUpperCase()}`;
      heading.append(stationCode);
      ...
      addText("span", station.status, `popup-status status-text status-${station.status.toLowerCase()}`);
      addText("small", `Pembaruan simulasi · ${latest.current.timeLabel}`, "popup-time");
      addText("small", `${station.delta3hM >= 0 ? "Naik" : "Turun"} ${Math.abs(station.delta3hM).toFixed(2)} m dalam 3 jam`, "popup-trend");
      const environment = document.createElement("div");
      environment.className = "popup-environment";
      const rain = document.createElement("span");
      rain.textContent = `Hujan 30 mnt · ${station.rainfall30mMm.toFixed(1)} mm`;
      const quality = document.createElement("span");
      quality.className = `quality-${station.quality.status.toLowerCase()}`;
      quality.textContent = `Kualitas · ${station.quality.status}`;
      environment.append(rain, quality);
      ...
      if (menu) {
        ...
        alertAction.innerHTML = '<svg class="popup-action-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9ZM10 21h4"/></svg><span>Buat notifikasi bahaya</span>';
        alertAction.setAttribute("aria-label", `Buat notifikasi bahaya untuk ${station.name}`);
        alertAction.onclick = () => { alertAction.innerHTML = '<svg class="popup-action-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="m5 12 4 4L19 6"/></svg><span>Notifikasi aktif</span>'; alertAction.disabled = true; };
        const chart = document.createElement("button"); chart.textContent = "Lihat grafik"; chart.onclick = () => { latest.current.onSelect(station); latest.current.onOpenChart(); popup.current?.remove(); }; actions.append(chart, detail, alertAction); content.append(actions);
      }
    ...
    map.once("style.load", () => {
      ...
      map.addSource("basin-mask",{type:"geojson",data:emptyGeoJSON});
      map.addLayer({id:"basin-mask",type:"fill",source:"basin-mask",layout:{visibility:latest.current.focusBasin && latest.current.region === "welang" ? "visible" : "none"},paint:{"fill-color":"#102d4e","fill-opacity":.45}});
    ...
    loadGeoAssets().then(data => {
      ...
      const apply = () => {
        ...
        setHasBasin(data.sources.basin);
        (map.getSource("basin-mask") as GeoJSONSource | undefined)?.setData(outsideMask(data.assets.basin));
        latest.current.onHydroSources(data.sources);
        if(frameAll.current) fitAll.current();
        if(popupStation.current) showPopup.current(popupStation.current.name,popupStation.current.menu);
      };
  ...
  useEffect(() => {
    ...
    if (!map || !ready) return;
    for (const [id, key] of [["regencies-fill", "admin"], ["regencies-outline", "admin"], ["east-river-lines", "regionalRivers"], ["basin-fill", "basin"], ["basin-outline", "basin"], ["subdas-fill", "subdas"], ["subdas-outline", "subdas"], ["river-lines", "topology"]] as const) {
      if (map.getLayer(id)) map.setLayoutProperty(id, "visibility", props.activeLayers.includes(key) ? "visible" : "none");
    }
  }, [props.activeLayers, ready]);
  useEffect(() => { const map=mapRef.current; if(map?.getLayer("basin-mask")) map.setLayoutProperty("basin-mask","visibility",props.focusBasin && props.region === "welang" ? "visible" : "none"); },[props.focusBasin,props.region,hasBasin,ready]);
  ...
  return <>
    <div className="map-canvas" ref={container} role="region" aria-label={props.region === "welang" ? "Peta stasiun DAS Welang" : "Peta sumber daya air Jawa Timur"} />
