...
export default function LegendPanel({ onClose, activeLayers, sources }: { onClose: () => void; activeLayers: LayerKey[]; sources: HydroSources }) {
  return <aside className="legend-card map-legend-overlay" role="region" aria-label="Legenda peta">
    <header className="panel-heading map-legend-heading"><button className="overlay-close" aria-label="Tutup legenda" onClick={onClose}><Icon name="close" />Tutup</button><h2>Legenda <Icon name="legend" /></h2></header>
    ...
    <details className="legend-details"><summary>Lapisan peta</summary><section>
      {activeLayers.includes("topology") && sources.topology && <div className="legend-row"><i className="legend-river" /><div><b>Sungai</b><small>{sources.topology ? "GeoJSON sungai lokal" : "Data sungai belum tersedia"}</small></div></div>}
      ...
      {!activeLayers.some(key => ["basin", "topology", "subdas", "admin", "regionalRivers"].includes(key)) && <small>Tidak ada overlay geospasial aktif.</small>}
    </section></details>
